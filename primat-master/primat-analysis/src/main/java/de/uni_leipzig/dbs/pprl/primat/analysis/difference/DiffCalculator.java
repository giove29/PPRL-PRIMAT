...
public class DiffCalculator {
	...
	/**
	 * Cost of an empty edit DiffOperation in terms of edit characters.
	 */
	 ...
	 *                    diff first to identify the changed areas. If true,
	...
	private LinkedList<Diff> diff_compute(String text1, String text2, boolean checklines, long deadline) {
		...
		if (i != -1) {
			...
			DiffOperation op = (text1.length() > text2.length()) ? DiffOperation.DELETE
	 ...
	 * @return       An object containing the encoded text1, the encoded text2
	 *               and the List of unique strings. The zeroth element of the
	 *               List of unique strings is intentionally blank.
	 */
		...
		while (lineEnd < text.length() - 1) {
			...
			if (lineHash.containsKey(line)) {
				...
				if (lineArray.size() == maxLines) {
					...
					// String.valueOf((char)
					// 65536).equals(String.valueOf(((char) 0)))
	...
	protected void diff_charsToLines(List<Diff> diffs, List<String> lineArray) {
		...
		for (Diff diff : diffs) {
			...
			for (int j = 0; j < diff.getText().length(); j++) {
	 ...
	 * @param  text1 First string.
	 * @param  text2 Second string.
	 * @return       Five element String array, containing the prefix of text1,
	 *               the suffix of text1, the prefix of text2, the suffix of
	 ...
	 * @param  i         Start index of quarter length substring within
	 *                   longtext.
	 * @return           Five element String array, containing the prefix of
	 *                   longtext, the suffix of longtext, the prefix of
	 *                   shorttext, the suffix of shorttext and the common
	...
	public void diff_cleanupSemantic(LinkedList<Diff> diffs) {
		...
		Deque<Diff> equalities = new ArrayDeque<Diff>(); // Double-ended queue
															// of qualities.
			...
			if (prevDiff.getOperation() == DiffOperation.DELETE && thisDiff.getOperation()== DiffOperation.INSERT) {
				...
				if (overlap_length1 >= overlap_length2) {
					...
					if (overlap_length1 >= deletion.length() / 2.0 || overlap_length1 >= insertion.length() / 2.0) {
						...
						prevDiff.setText(deletion.substring(0, deletion.length() - overlap_length1));
					...
					if (overlap_length2 >= deletion.length() / 2.0 || overlap_length2 >= insertion.length() / 2.0) {
						...
						prevDiff.setText(insertion.substring(0, insertion.length() - overlap_length2));
	...
	/**
	 * Reduce the number of edits by eliminating DiffOperationally trivial
	 * equalities.
	 *
	 * @param diffs LinkedList of Diff objects.
	 */
	public void diff_cleanupEfficiency(LinkedList<Diff> diffs) {
		...
		String lastEquality = null; // Always equal to equalities.peek().text
		ListIterator<Diff> pointer = diffs.listIterator();
		// Is there an insertion DiffOperation before the last equality.
		boolean pre_ins = false;
		// Is there a deletion DiffOperation before the last equality.
		boolean pre_del = false;
		// Is there an insertion DiffOperation after the last equality.
		boolean post_ins = false;
		// Is there a deletion DiffOperation after the last equality.
		...
		while (thisDiff != null) {
			...
			if (thisDiff.getOperation() == DiffOperation.EQUAL) {
				...
				if (lastEquality != null
					...
					pointer.set(new Diff(DiffOperation.DELETE, lastEquality));
					// Insert a corresponding an insert.
					pointer.add(thisDiff = new Diff(DiffOperation.INSERT, lastEquality));
	...
	public void diff_cleanupMerge(LinkedList<Diff> diffs) {
		diffs.add(new Diff(DiffOperation.EQUAL, "")); // Add a dummy entry at
														// the end.
		ListIterator<Diff> pointer = diffs.listIterator();
		int count_delete = 0;
		int count_insert = 0;
				...
				case EQUAL:
					if (count_delete + count_insert > 1) {
						...
						if (both_types) {
							...
							if (commonlength != 0) {
								thisDiff = pointer.next();
								thisDiff.setText(text_insert.substring(text_insert.length() - commonlength)
									+ thisDiff.getText());
	...
	/**
	 * Crush the diff into an encoded string which describes the DiffOperations
	 * required to transform text1 into text2. E.g. =3\t-2\t+ing -> Keep 3
	 * chars, delete 2 chars, insert 'ing'. DiffOperations are tab-separated.
	...
	public String diff_toDelta(List<Diff> diffs) {
		...
		for (Diff aDiff : diffs) {
			...
			switch (aDiff.getOperation()) {
				case INSERT:
					try {
						text.append("+").append(URLEncoder.encode(aDiff.getText(), "UTF-8").replace('+', ' ')).append("\t");
					}
		...
		if (delta.length() != 0) {
			...
			delta = DiffMatchPatchUtils.unescapeForEncodeUriCompatability(delta);
		}
		return delta;
	}
	 ...
	 * DiffOperations required to transform text1 into text2, compute the full
	 * diff.
	 *
	 * @param  text1                    Source string for the diff.
	 * @param  delta                    Delta text.
	 * @return                          Array of Diff objects or null if
	 *                                  invalid.
	...
	public LinkedList<Diff> diff_fromDelta(String text1, String delta) throws IllegalArgumentException {
		...
		for (String token : tokens) {
			...
			// the
			// DiffOperation of this token (delete, insert, equality).
			...
			switch (token.charAt(0)) {
				...
				default:
					...
					throw new IllegalArgumentException(
						"Invalid diff DiffOperation in diff_fromDelta: " + token.charAt(0));
			}
		}
	...
	protected void patch_addContext(Patch patch, String text) {
		...
		if (prefix.length() != 0) {
			patch.diffs.addFirst(new Diff(DiffOperation.EQUAL, prefix));
		}
		// Add the suffix.
		...
		if (suffix.length() != 0) {
			patch.diffs.addLast(new Diff(DiffOperation.EQUAL, suffix));
		}
	 ...
	 * @param      diffs Array of Diff objects for text1 to text2.
	 * @return           LinkedList of Patch objects.
	 * @deprecated       Prefer patch_make(String text1, LinkedList<Diff>
	 *                   diffs).
	 */
	@Deprecated
	...
	public LinkedList<Patch> patch_make(String text1, LinkedList<Diff> diffs) {
		...
		for (Diff aDiff : diffs) {
			...
			if (patch.diffs.isEmpty() && aDiff.getOperation() != DiffOperation.EQUAL) {
			...
			switch (aDiff.getOperation()) {
				case INSERT:
					...
					patch.length2 += aDiff.getText().length();
					postpatch_text = postpatch_text.substring(0, char_count2) + aDiff.getText()
						+ postpatch_text.substring(char_count2);
					break;
				case DELETE:
					patch.length1 += aDiff.getText().length();
					patch.diffs.add(aDiff);
					postpatch_text = postpatch_text.substring(0, char_count2)
						+ postpatch_text.substring(char_count2 + aDiff.getText().length());
					break;
				case EQUAL:
					if (aDiff.getText().length() <= 2 * Patch_Margin && !patch.diffs.isEmpty() && aDiff != diffs.getLast()) {
					...
					if (aDiff.getText().length() >= 2 * Patch_Margin && !patch.diffs.isEmpty()) {
	 ...
	 * @param  patches Array of Patch objects
	 * @param  text    Old text.
	 * @return         Two element Object array, containing the new text and an
							...
							if (aDiff.getOperation() != DiffOperation.EQUAL) {
								...
								if (aDiff.getOperation() == DiffOperation.INSERT) {
									...
									text = text.substring(0, start_loc + index2)
										+ text.substring(start_loc + diff_xIndex(diffs, index1 + aDiff.getText().length()));
								}
							}
	...
	public String patch_addPadding(LinkedList<Patch> patches) {
		...
		if (diffs.isEmpty() || diffs.getFirst().getOperation() != DiffOperation.EQUAL) {
			// Add nullPadding equality.
			diffs.addFirst(new Diff(DiffOperation.EQUAL, nullPadding));
		...
		if (diffs.isEmpty() || diffs.getLast().getOperation() != DiffOperation.EQUAL) {
			// Add nullPadding equality.
			diffs.addLast(new Diff(DiffOperation.EQUAL, nullPadding));
			patch.length1 += paddingLength;
			patch.length2 += paddingLength;
		}
		...
		while (bigpatch != null) {
			...
			while (!bigpatch.diffs.isEmpty()) {
				...
				if (precontext.length() != 0) {
					...
					patch.diffs.add(new Diff(DiffOperation.EQUAL, precontext));
				}

				while (!bigpatch.diffs.isEmpty() && patch.length1 < patch_size - Patch_Margin) {
					diff_type = bigpatch.diffs.getFirst().getOperation();
					diff_text = bigpatch.diffs.getFirst().getText();
					...
					if (diff_type == DiffOperation.INSERT) {
					...
					else if (diff_type == DiffOperation.DELETE && patch.diffs.size() == 1
						&& patch.diffs.getFirst().getOperation() == DiffOperation.EQUAL
						...
						if (diff_text.equals(bigpatch.diffs.getFirst().getText())) {
							bigpatch.diffs.removeFirst();
						}
						else {
							final String newText = bigpatch.diffs.getFirst().getText().substring(diff_text.length());
							bigpatch.diffs.getFirst().setText(newText);
						}
					}
				}
				...
				if (postcontext.length() != 0) {
					...
					if (!patch.diffs.isEmpty() && patch.diffs.getLast().getOperation() == DiffOperation.EQUAL) {
						patch.diffs.getLast().setText(patch.diffs.getLast().getText() + postcontext);
					}
					else {
						patch.diffs.add(new Diff(DiffOperation.EQUAL, postcontext));
					}
				}
	...
	public List<Patch> patch_fromText(String textline) throws IllegalArgumentException {
		...
		while (!text.isEmpty()) {
			...
			while (!text.isEmpty()) {
				...
				if (sign == '-') {
					// Deletion.
					patch.diffs.add(new Diff(DiffOperation.DELETE, line));
				}
				else if (sign == '+') {
					// Insertion.
					patch.diffs.add(new Diff(DiffOperation.INSERT, line));
				}
				else if (sign == ' ') {
					// Minor equality.
					patch.diffs.add(new Diff(DiffOperation.EQUAL, line));
				}
				else if (sign == '@') {
					// Start of next patch.
					break;
