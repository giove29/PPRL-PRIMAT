...
class FieldMergerTest {
	...
	void test() {
		...
		MergeDefinition mergeDef = new MergeDefinition();
		mergeDef.setMerger(new DotMerger(), "DOB", "MOB", "YOB");
		FieldMerger merger = new FieldMerger(mergeDef);
		merger.preprocess(record);
		...
		assertEquals("20.07.1975", record.getQidAttribute(2).getStringValue());
		assertEquals("04109 Leipzig", record.getQidAttribute(3).getStringValue());
		assertEquals("M", record.getQidAttribute(4).getStringValue());	
		
		merger.updateSchema();
		
		assertEquals("Peter", record.getQidAttribute("FN").getStringValue());
		assertEquals("Schmidt", record.getQidAttribute("LN").getStringValue());
		assertEquals("20.07.1975", record.getQidAttribute("DOB_MOB_YOB").getStringValue());
		assertEquals("04109 Leipzig", record.getQidAttribute("CITY").getStringValue());
		assertEquals("M", record.getQidAttribute("GENDER").getStringValue());		
		
		assertEquals(5, RecordSchema.INSTANCE.getNameColumnMapping().size());
		assertEquals(5, RecordSchema.INSTANCE.getNameAttributeTypeMapping().size());	
	}
