 ...
 *
 */
public class EnhancedDoubleHashing extends HashingMethod {
	 ...
	 * @param bfSize
	 */
	public EnhancedDoubleHashing(int bfSize) {
	 ...
	 * @param hash2
	 */
	public EnhancedDoubleHashing(int bfSize, String salt, MessageDigestAlgorithm hash1, MessageDigestAlgorithm hash2) {
		super(bfSize);
		this.salt = salt;
		this.hash1 = hash1;
		this.hash2 = hash2;
	...
	public List<Integer> hashToInts(String element, int hashFunctions) {
		...
		for (int hashNumber = 0; hashNumber < hashFunctions; hashNumber++) {
			final int value = hashValue1 + hashNumber * hashValue2 + (int) ((Math.pow(hashNumber, 3) - hashNumber) / 6);
