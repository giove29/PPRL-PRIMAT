...
- opzionalmente, top-level, `"missingValueHandling": {"enabled", "anchorPriority": [...]}`: se abilitato, un attributo QID vuoto viene codificato con token sintetici scelti su un bucket deterministico (invece del padding fisso `"___"`), derivato dall'hash del primo attributo QID non vuoto del record secondo l'ordine di `anchorPriority`
