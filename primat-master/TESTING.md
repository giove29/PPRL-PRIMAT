...
- **Istogramma delle similarità** (`python_evaluation/plot_similarity_histogram.py`): con `debug: true` nel JSON della LU, il run scrive `python_evaluation/similarity_histogram.csv` (percorso relativo alla root, dove si lancia la LU; colonne `truth,bin_low,bin_high,count`, `truth` = match vero/non-match secondo il `GLOBAL_ID`; un solo istogramma per contesto, senza divisione cross/within-party); il grafico (default: CSV e PNG accanto allo script, da qualunque directory) si genera con:
  ...
  ```
- **Soglia automatica** (`"similarityThreshold": "auto" | "auto_precision" | "auto_recall"`, vedi `SOTA_SOGLIA_DINAMICA.md` e `primat-linkage-unit-service/src/main/resources/config/examples/full_reference.json`): la LU fa una passata di profilo, stima il ginocchio tra non-match e match, lo sposta di `autoThreshold.epsilon` (default 0.03) e stampa riga `Soglia:` con IC95, regime, affidabilita' e stime di precision/recall. Con `debug: true` scrive anche `python_evaluation/similarity_threshold.csv` (`key,value`) che `plot_similarity_histogram.py` disegna come soglia con banda IC (`python python_evaluation/plot_similarity_histogram.py python_evaluation/similarity_histogram.csv -`). Benchmark contro la soglia oracolo (ground truth) su istogrammi reali: `mvn -q -o -pl primat-linkage-unit-service exec:java@threshold-benchmark -Dexec.args="<keySize> <keys> python_evaluation/similarity_histogram.csv"` (keySize/keys = r/b del blocking con cui e' stato generato il CSV).
