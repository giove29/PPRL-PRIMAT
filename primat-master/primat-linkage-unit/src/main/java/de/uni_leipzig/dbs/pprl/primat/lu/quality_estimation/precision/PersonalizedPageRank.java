...
import java.util.*;
     ...
     * caller. The default value is {@link PersonalizedPageRank#MAX_ITERATIONS_DEFAULT}.
     * </p>
     *
     * <p>
            ...
            VertexScoringAlgorithm<V, Double>
    {
         ...
         */
        public static final double TOLERANCE_DEFAULT = 1e-6;
        ...
        public V getPersonalNode() {
            return personalNode;
        }

        public void setPersonalNode(V personalNode) {
            this.personalNode = personalNode;
        }

        private V personalNode;
        /**
         * Create and execute an instance of PageRank.
         *
         * @param graph the input graph
         */
        public PersonalizedPageRank(Graph<V, E> graph)
        {
         ...
         */
        public PersonalizedPageRank(Graph<V, E> graph, double dampingFactor)
        {
         ...
         */
        public PersonalizedPageRank(Graph<V, E> graph, double dampingFactor, int maxIterations)
        {
         ...
         */
        public PersonalizedPageRank(Graph<V, E> graph, double dampingFactor, int maxIterations, double tolerance)
        {
        ...
        {
            if (scores == null) {
                scores = Collections.unmodifiableMap(new Algorithm(personalNode).getScores());
            }
            return scores;
        }
        ...
        private class Algorithm
        {
            ...
            public Algorithm(V personalVertex)
            {
                ...
                this.personScore = new double[totalVertices];
                ...
                for (V v : graph.vertexSet()) {
                    ...
                    if(!v.equals(personalVertex)) {
                        curScore[i] = initScore/((totalVertices-1)*0.01+1);
                        personScore[i] = initScore/((totalVertices-1)*0.01+1);
                    } else {
                        curScore[i] = 1/((totalVertices-1)*0.01+1);
                        personScore[i] = 1/((totalVertices-1)*0.01+1);
                    }
                    i++;
                }
                if (isWeighted) {
            ...
            private void run()
            {
                ...
                while (iterations > 0 && maxChange >= tolerance) {
                    ...
                    for (int i = 0; i < totalVertices; i++) {
                        ...
                        double vNewValue = r[i] + contribution;
            ...
            private void runWeighted()
            {
                ...
                while (iterations > 0 && error > (tolerance * totalVertices)) {
                    double[] r = teleProp();
                    error=0;
                    maxChange = 0d;
                    for (int i = 0; i < totalVertices; i++) {
                        ...
                        for (int j = 0, getLength = neighbors.length; j < getLength; j++) {
                            int w = neighbors[j];
                            contribution += curScore[w] * weights[j] / weightSum[w];
                        }

                        double vOldValue = curScore[i];
                        double vNewValue = r[i] + dampingFactor * contribution;
                        maxChange = Math.max(maxChange, Math.abs(vNewValue - vOldValue));
                        error += Math.abs(vNewValue - vOldValue);
                        nextScore[i] = vNewValue;
                    }
            ...
            {
                double[] r = new double[totalVertices];
                for (int i = 0; i < totalVertices; i++) {
//                    if (outDegree[i] > 0) {
                    r[i] = (1d - dampingFactor) * personScore[i];
//                    } else {
//                        r[i] = personScore[i];
//                    }
                }
