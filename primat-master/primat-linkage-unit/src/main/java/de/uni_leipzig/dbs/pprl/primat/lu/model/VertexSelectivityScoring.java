 ...
 * @param  <E>
 */
public class VertexSelectivityScoring<V, E> implements VertexScoringAlgorithm<V, Double> {

	private final Graph<V, E> graph;
	private Map<V, Double> scores;

	public VertexSelectivityScoring(Graph<V, E> graph) {
	...
	private void compute() {
		...
		for (final V vertex : vertices) {
			...
			if (edges.size() == 0) {
				...
				score = this.graph.getEdgeWeight(edges.get(0));
			}
			else {
