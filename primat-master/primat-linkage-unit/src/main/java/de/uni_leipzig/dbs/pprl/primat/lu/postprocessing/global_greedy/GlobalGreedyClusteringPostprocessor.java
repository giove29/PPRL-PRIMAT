 ...
 */
package de.uni_leipzig.dbs.pprl.primat.lu.postprocessing.global_greedy;
...
/**
 * Wiring di {@link PriorityClusteringEngine} (con priorità = similarità
 * grezza) su {@link MultiPartiteSimilarityGraph}, analogo a
 * {@code CenterClusteringPostprocessor}. Generalizzazione N-aria,
 * source-consistent, del "Global Greedy" bipartito di
 * {@code GreedyPostprocessor} (ordina tutti gli archi per similarità
 * decrescente e li assegna greedily, qui con vincolo di un solo record per
 * party per cluster invece che una sola coppia per volta): dedicata allo
 * scenario in cui tutte le sorgenti sono Clean.
 */
public class GlobalGreedyClusteringPostprocessor implements MultipartiteClusteringStrategy {

	private final GlobalGreedyConfig config;

	public GlobalGreedyClusteringPostprocessor(GlobalGreedyConfig config) {
		this.config = config;
	}
	...
	public List<LinkedPair<Record>> cluster(MultiPartiteSimilarityGraph graph) {
		...
		for (final MultiPartiteSimilarityGraph component : ProgressListener.tracking(graph.getConnectedComponentsSimGraph(), progressListener)) {
			...
			orderedRecords.sort(Comparator.comparing(GlobalGreedyClusteringPostprocessor::stableKey));
			...
			final PriorityClusteringEngine engine = new PriorityClusteringEngine(simMatrix, partyIds,
					orderedParties.size(), EdgePriorityFunction.RAW_SIMILARITY, config.getMergeThreshold());
			engine.fit();
